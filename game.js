(() => {
  ...
  const PLAYER_FIRE_INTERVAL = 1.0;
  ...
  function activeEffectLabel(tank) {
    const labels = [];
    if (state.simTime < tank.effects.speedUntil) labels.push(`Speed ${Math.max(0, tank.effects.speedUntil - state.simTime).toFixed(1)}s`);
    if (state.simTime < tank.effects.rapidUntil) labels.push(`Rapid ${Math.max(0, tank.effects.rapidUntil - state.simTime).toFixed(1)}s`);
  ...
  function updateHud() {
    ...
    el.difficultyLabel.textContent = state.difficulty ? `Difficulty: ${DIFFICULTIES[state.difficulty].label}` : 'Difficulty: —';
    ...
    el.fireStatus.textContent = tankA.fireCooldown > 0 ? `${tankA.fireCooldown.toFixed(1)}s cooldown` : 'Ready';
  ...
  function resetTank(tank) {
    ...
    tank.vy = 0;
    tank.hp = MAX_HP;
    tank.fireCooldown = isPlayer ? 0 : Math.min(0.75, tank.baseFireInterval * 0.5);
  ...
  function resetRound() {
    ...
    setStatePanel(
      `ROUND ${state.round}`,
      'Destroy the opposing tank. The cyan center line blocks tanks, not projectiles. Power-ups spawn in mirrored pairs so both sides get the same opportunity.'
    );
    updateHud();
  }
  ...
  function restartMatch() {
    ...
    setStatePanel(
      'TANK PONG',
      'Destroy the opposing tank while evading its fire. Each tank has 3 HP; first to win 3 rounds takes the best-of-five match.',
      'difficulty'
    );
    updateHud();
  }
  ...
  function togglePause() {
    ...
    if (state.paused) {
      setStatePanel('PAUSED', 'Gameplay, cooldowns, AI, projectiles, and active power-up timers are frozen. Press P or Resume to continue.');
    } else {
      setStatePanel(`ROUND ${state.round}`, 'Combat resumed. Destroy the opposing tank before it destroys you.');
      state.lastFrame = performance.now();
    }
    updateHud();
  }
  ...
  function findThreat(config) {
    ...
    for (const projectile of projectiles) {
      ...
      const dangerRadius = Math.hypot(tankB.width, tankB.height) * 0.45 + projectile.radius + config.dangerPadding;
      if (miss > dangerRadius) continue;

      if (!best || time < best.time) {
        best = { projectile, time, projectedX, projectedY, miss };
      }
    }
    return best;
  }
  ...
  function boundedDodgeVector(projectile) {
    ...
    for (const option of options) {
      const futureX = tankB.x + option.x * 100;
      const futureY = tankB.y + option.y * 100;
      const xRoom = Math.min(futureX - (CX + tankB.width / 2), (W - tankB.width / 2) - futureX);
      const yRoom = Math.min(futureY - tankB.height / 2, (H - tankB.height / 2) - futureY);
      const score = Math.min(xRoom, yRoom) + (xRoom > margin ? 15 : 0) + (yRoom > margin ? 15 : 0);
      if (score > bestScore) {
        bestScore = score;
        best = option;
      }
  ...
  function chooseAiIntent(config) {
    ...
    if (threat) {
      ...
      if (Math.random() < config.dodgeMistake) {
        dodge = Math.random() < 0.5 ? { x: 0, y: Math.random() < 0.5 ? -1 : 1 } : { x: -dodge.x, y: -dodge.y };
      }
    ...
    if (tankB.ai.wanderTimer <= 0 || Math.hypot(tankB.ai.wanderTargetX - tankB.x, tankB.ai.wanderTargetY - tankB.y) < 35) {
      ...
      tankB.ai.wanderTimer = 0.6 + Math.random() * 1.2;
    }
  ...
  function updateProjectiles(dt) {
    for (let i = projectiles.length - 1; i >= 0; i--) {
      ...
      projectile.trail.push({ x: projectile.x, y: projectile.y, life: 0.12 });
      if (projectile.trail.length > 7) projectile.trail.shift();
      for (const point of projectile.trail) point.life -= dt;
  ...
  function finishRound(winner) {
    ...
    state.phase = winner === 'A' && state.scoreA + 1 >= MATCH_WINS || winner === 'B' && state.scoreB + 1 >= MATCH_WINS
      ? 'matchExplosion'
      : 'roundExplosion';
    ...
    el.pauseButton.disabled = true;
    setStatePanel(
      winner === 'A' ? 'ENEMY TANK DESTROYED' : 'YOUR TANK WAS DESTROYED',
      'The destruction animation must finish before the match can continue.'
    );
    updateHud();
  }

  function updateTransition(dt) {
    if (state.phase !== 'roundExplosion' && state.phase !== 'matchExplosion') return;
    ...
    if (state.phase === 'matchExplosion') {
      ...
      setStatePanel(
        playerWon ? 'YOU WIN THE MATCH' : 'AI WINS THE MATCH',
      ...
      setStatePanel(
        state.roundWinner === 'A' ? `ROUND ${state.round}: YOU WIN` : `ROUND ${state.round}: AI WINS`,
  ...
  function updatePowerups(dt) {
    ...
    for (const powerup of powerups) powerup.pulse += dt * 4;
  }

  function applyPowerup(tank, type) {
    if (type === 'speed') {
      tank.effects.speedUntil = Math.max(tank.effects.speedUntil, state.simTime) + POWERUP_DURATION;
    } else if (type === 'rapid') {
      tank.effects.rapidUntil = Math.max(tank.effects.rapidUntil, state.simTime) + POWERUP_DURATION;
    } else if (type === 'shield') {
      tank.effects.shield = 1;
    }
    createExplosion(tank.x, tank.y, COLORS[type], 16, 0.4);
    playSound('powerup');
  }
  ...
  function drawProjectile(projectile) {
    ...
    for (const point of projectile.trail) {
      const alpha = clamp(point.life / 0.12, 0, 1) * 0.32;
      ctx.globalAlpha = alpha;
      ctx.fillStyle = projectile.color;
      ctx.beginPath();
      ctx.arc(point.x, point.y, projectile.radius * 0.75, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.globalAlpha = 1;
    ctx.fillStyle = projectile.color;
  ...
  function gameLoop(now) {
    ...
    if (!state.paused) {
      ...
      if (state.phase === 'roundExplosion' || state.phase === 'matchExplosion') {
        updateTransition(dt);
      }
  ...
  canvas.addEventListener('pointerdown', event => {
    ...
    if (event.button !== 0 || state.phase !== 'playing' || state.paused) return;
  ...
  el.soundButton.addEventListener('click', () => {
    ...
    if (!state.muted) playSound('powerup');
    updateHud();
  });
  el.continueButton.addEventListener('click', nextRound);
